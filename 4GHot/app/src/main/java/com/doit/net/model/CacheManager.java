...
//        }
    ...
    public static void changeBand( String idx,  String changeBand) {
        ...
        new Timer().schedule(new TimerTask() {
            @Override
            public void run() {
                DBChannel dbChannel = ProtocolManager.getCheckedFcn(changeBand);
                if (dbChannel != null && !TextUtils.isEmpty(dbChannel.getFcn()) && !TextUtils.isEmpty(dbChannel.getPlmn())){
                    ProtocolManager.setChannelConfig(idx, dbChannel.getFcn(), dbChannel.getPlmn(), "", "", "", "", "");
                }
