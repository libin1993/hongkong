...
public class CustomFcnActivity extends BaseActivity {
    ...
    private void initView() {
        ...
        adapter = new BaseSectionQuickAdapter<SectionBean, BaseViewHolder>(R.layout.layout_fcn_item,
            ...
            protected void convert(BaseViewHolder helper, SectionBean item) {
                ...
                helper.setText(R.id.tv_fcn, "制式: " + channel.getPlmn() + "\n频点: " + channel.getFcn());
                ...
                switch (view.getId()) {
                    case R.id.iv_add_fcn:
                        ...
                        AddFcnDialog addFcnDialog = new AddFcnDialog(CustomFcnActivity.this,
                                "添加制式、频点",sectionBean.header, "", "", new AddFcnDialog.OnConfirmListener() {
                            @Override
                            public void onConfirm(String plmn, String fcn) {
                                addFcn(position, sectionBean.header,  plmn, fcn);
                            }
                    ...
                    case R.id.ll_select_fcn:
                        ...
                        checkFcn(section.t.getBand(), section.t.getPlmn(), section.t.getFcn());
                        break;
                    case R.id.ll_edit_fcn:
                        AddFcnDialog editFcnDialog = new AddFcnDialog(CustomFcnActivity.this,
                                "编辑制式、频点", dataList.get(position).t.getBand(), dataList.get(position).t.getPlmn(),
                                dataList.get(position).t.getFcn(), new AddFcnDialog.OnConfirmListener() {
                            @Override
                            public void onConfirm(String plmn, String fcn) {
                                SectionBean sectionBean = dataList.get(position);
                                update(position, sectionBean.t.getId(), sectionBean.t.getBand(), plmn, fcn);
                            }
                        });
                        editFcnDialog.show();
                        break;
                    case R.id.ll_delete_fcn:
                        ...
                        deleteDcn(position, sectionDelete.t.getBand(), sectionDelete.t.getPlmn(),
                                sectionDelete.t.getFcn(), sectionDelete.t.isCheck());
                        break;
                }
            }
        });
     ...
     * 设置默认fcn
     */
    private void checkFcn(String band, String plmn, String fcn) {
        for (int i = 0; i < dataList.size(); i++) {
            SectionBean sectionBean = dataList.get(i);
            if (!sectionBean.isHeader && band.equals(sectionBean.t.getBand())) {
                if (plmn.equals(sectionBean.t.getPlmn()) && fcn.equals(sectionBean.t.getFcn())) {
        ...
        try {
            ...
            for (DBChannel channel : dbChannel) {
                if (plmn.equals(channel.getPlmn()) && fcn.equals(channel.getFcn())) {
                    channel.setCheck(1);
                } else {
                    channel.setCheck(0);
                }
     ...
     * @param fcn      编辑fcn
     */
    private void update(int position, int id, String band, String plmn, String fcn) {
        for (int i = 0; i < dataList.size(); i++) {
            SectionBean sectionBean = dataList.get(i);
            if (!sectionBean.isHeader && band.equals(sectionBean.t.getBand()) && plmn.equals(sectionBean.t.getPlmn())
                    && fcn.equals(sectionBean.t.getFcn())) {
                ToastUtils.showMessage("已存在相同制式、频点");
                return;
            }
        }

        dataList.get(position).t.setPlmn(plmn);
     ...
     * 删除fcn
     */
    private void deleteDcn(int position, String band, String plmn,String fcn, int isCheck) {
        //删除已选中的，设置默认fcn为选中状态
     ...
     * @param fcn  新增fcn
     */
    private void addFcn(int position,String band, String plmn, String fcn) {
        ...
        DBChannel dbChannel = new DBChannel( band, fcn, plmn, 0, 0);
